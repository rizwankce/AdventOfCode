...
func partOne() -> Int {
    ...
    func validate(_ str: String) -> Bool {
        let com = str.components(separatedBy: " ")
        return Set(com).count == com.count
    }
...
func partTwo() -> Int {
    ...
    func validate(_ str: String) -> Bool {
        func isAnagram(_ line: String,_ com: [String]) -> Bool {
            var valid = false
            for c in com {
        ...
        let com = str.components(separatedBy: " ")
        if Set(com).count != com.count {
            return false
        }
        
        var containsAnagram = false
        for line in com {
            let rem = com.filter { $0 != line }
            if isAnagram(line, rem) {
