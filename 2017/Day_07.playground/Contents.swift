...
var tower: [String: [String]] = [:]
var weights: [String: Int] = [:]
for line in input {
    if line.contains("->") {
        let com = line.components(separatedBy: " -> ")
        let parant = com[0].components(separatedBy: " ")[0]
        let childs = com[1].components(separatedBy: ", ")
        tower[parant] = childs
        
        let value = com[0].components(separatedBy: " ")[1].replacingOccurrences(of: "(", with: "")
        let w = value.replacingOccurrences(of:")", with: "")
        weights[parant] = Int(w)!
    }
    else {
        let com = line.components(separatedBy: " ")
        let parent = com[0]
        let value = com[1].replacingOccurrences(of: "(", with: "")
        let w = value.replacingOccurrences(of: ")", with: "")
        weights[parent] = Int(w)!
    }
}
...
func partTwo() -> Int {
    ...
    func check(_ key: String) -> Disk {
        guard tower.keys.contains(key) else {
            return Disk(name: key, weight: weights[key]!, isBalanced: true)
//            return (weights[key]!,true)
        }
        let aboves = tower[key]!
        let sub = aboves.map { check($0) }
        let ws = Set(sub.map { $0.weight })
        let isBalanced = ws.count <= 1
        let totalWeight = weights[key]! + sub.map { $0.weight }.reduce(0, +)
        
        let subBalanced = Set(sub.map {$0.isBalanced } ).first!
        //print(sub.map {$0.1 })
        if ws.count > 1 && subBalanced {
            print(key,totalWeight,isBalanced,ws)
            let diff = ws.max()! - ws.min()!
//            print(diff)
            for pair in sub {
                let ab = tower[key]!
                print(ab,pair.weight,key)
               
                if ws.max()! == pair.weight {
                    print(pair)
                    print(weights[pair.name]! - diff)
                    result = weights[pair.name]! - diff
                }
            }
        }
        return Disk(name: key, weight: totalWeight, isBalanced: isBalanced)
    }
