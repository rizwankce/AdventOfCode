...
func partOne() -> Int {
    ...
    let given = paths.filter { line in
...
func partTwo() -> Int {
    ...
    func isDiagnoal(l1:Point, l2:Point) -> Bool {
        l1.x + l1.y
        == l2.x + l2.y || max(l1.x, l1.y) - min(l1.x, l1.y) == max(l2.x, l2.y) - min(l2.x, l2.y)
    }
    ...
    for g in given {
        ...
        else if isDiagnoal(l1: l1, l2: l2) {
            let diff = abs(l1.x - l2.x)
            let xIncreases = l2.x > l1.x
            let yIncreases = l2.y > l1.y
            (0 ... diff).forEach { i in
                let p = Point(
                    x: l1.x + (xIncreases ? i : -i),
                    y: l1.y + (yIncreases ? i : -i)
                )
                grid[p] = grid[p]! + 1
            }
        }
