    ...
    public func run() {
        ...
        while true {
            ...
            case 3:
                ...
                setValue(p1, inputs.removeFirst(),getMode(2))
...
let addresses = 0..<50
let computers: [Opcode] = addresses.map { i in
    print("c i \(i)")
    let computer = Opcode(input)
    computer.inputs.append(i)
    computer.run()
    return computer
}
var inputs: [[Int]] = addresses.map { i in return [i]}

func getInput(_ i: Int) -> Int {
    print(inputs[i])
    if !inputs[i].isEmpty {
        return inputs[i].removeFirst()
    }
    return -1
}
while true {
    for (i,computer) in computers.enumerated() {
        ...
        computer.run()
        if computer.outputs.count  >= 3 {
            let addr = computer.outputs[0]
            let x = computer.outputs[1]
            let y = computer.outputs[2]
            ...
            else if addr < 50 || addr >= 0{
                computers[addr].outputs.append(y)
                computers[addr].outputs.append(x)
            }
        }
    }
}
