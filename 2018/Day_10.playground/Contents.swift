...
struct Point: CustomStringConvertible, Equatable, Hashable {
    ...
    func move(by point: Point, time: Int) -> Point {
        Point.init(x: self.x+(point.x*time), y: self.y+(point.y*time))
    }
}
...
struct Grid {
    ...
    init(_ points: [Point], _ velocities: [Point]) {
        self.points = points
    ...
    mutating func debugPrint() {
        ...
        for r in (minX ... maxX) {
            for c in (minY ... maxY) {
                let p = Point(x: r, y: c)
                print(points.contains(p) ? "#" : " ", separator: " ", terminator: " ")
            }
            print("\n")
        }
        print("grid end")
    ...
    mutating func tick(_ time: Int) {
        var next: [Point] = []
        for i in 0 ..< points.count {
            let p = points[i]
            let v = velocities[i]

            let new = p.move(by: v, time: time)
            next.append(new)
        }

        let xMin = next.min(by: { $0.x < $1.x })!.x
        let yMin = next.min(by: { $0.y < $1.y })!.y

        for (index,_) in next.enumerated() {
            next[index].x -= xMin
            next[index].y -= yMin
        }
...
for line in input {
    ...
    let point = Point.init(x: numbers[1], y: numbers[0])
    points.append(point)
    let velocity = Point.init(x: numbers[3], y: numbers[2])
    velocities.append(velocity)
}

func partOne() -> Int {
    for t in 1... {
        var grid = Grid.init(points, velocities)
        grid.tick(t)
        if t == 10054 {
            grid.debugPrint()
            break
