...
""".components(separatedBy: .newlines).sorted()
...
class Day {
    ...
    var start: Int? = nil
    var end: Int? = nil
    var slots: [Int] = Array.init(repeating: 0, count: 60)
...
for line in input {
    ...
    if line.contains("Guard") {
        ...
        let id = components[3].dropFirst()
//        print(id)
        let day = Day.init(Int(String(id))!)
        days.append(day)
    }

    if line.contains("falls") {
        let time = components[1].dropLast().components(separatedBy: ":").last
//        print(time)
        var day = days.last!
        day.start = Int(time!)!
//        days.append(day)
...
func partOne() {
    ...
    //    print(day.id)
    //    print(day.slots)
        guards[day.id] = guards[day.id, default: 0] + day.slots.filter { $0 == 1 }.count
    }
    let max = guards.max { $0.value < $1.value }!
    print(max.key)
    let id = max.key

    var sleep: [Int] = Array.init(repeating: 0, count: 60)
    for day in days {
        if day.id == id {
            day.slots.enumerated().forEach { (i,v) in
                sleep[i] += v == 1 ? 1 : 0
            }
        }
    }
    let x = id
    let y = sleep.firstIndex(of: sleep.max()!)!
    print("Part 1 Answer is : \(x * y))")
}

func partTwo() {
    var sleep: [Int: [Int]] = [:]
    let empty: [Int] = Array.init(repeating: 0, count: 60)
    for day in days {
        let id = day.id
        var min = sleep[id, default:empty]
        for (i,v) in day.slots.enumerated() {
            min[i] += v == 1 ? 1 : 0
        }
        sleep[id] = min
    }
    ...
    for s in sleep {
        let max = s.value.max()!
        let i = s.value.firstIndex(of: max)!
        result[s.key] = (i,max)
    }

    let dict = result.max(by: { $0.value.1 < $1.value.1})!
    let x = dict.key
    let y = dict.value.0
    print(dict)
    print("Part Two answer is :\(x*y)")
}
