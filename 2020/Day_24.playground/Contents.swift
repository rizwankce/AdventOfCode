...
struct Tile: CustomStringConvertible, Equatable, Hashable {
    ...
    func neighbour(_ direction: Direction) -> Tile {
        switch direction {
        case .east: return Tile.init(x: x+1, y: y)
        case .west: return Tile.init(x: x-1, y: y)
        case .southWest: return Tile.init(x: x-1, y: y-1)
        case .southEast: return Tile.init(x: x, y: y-1)
        case .northWest: return Tile.init(x: x, y: y+1)
        case .northEast: return Tile.init(x: x+1, y: y+1)
        }
    }

    func allNeighbours() -> [Tile] {
        return Direction.allCases.map { neighbour($0) }
    }
}

let directions: [[Direction]] = []

var activeTitles: Set<Tile> = []

for line in input {
    var moves = line
    var position = Tile.init(x: 0, y: 0)

    while !moves.isEmpty {
        if ["s","n"].contains(moves.first) {
            let curMove = moves[moves.startIndex ... moves.index(after: moves.startIndex)]
            let dir = Direction.init(rawValue: String(curMove))!
            position = position.neighbour(dir)
            moves = String(moves.dropFirst(2))
        }
        else {
            let dir = Direction.init(rawValue: String(moves.first!))!
            position = position.neighbour(dir)
            moves = String(moves.dropFirst(1))
        }
    }

    if activeTitles.contains(position) {
        activeTitles.remove(position)
    }
    else {
        activeTitles.insert(position)
    }
...
func partTwo() -> Int {
    ...
    for _ in (1...100) {
        var nCount: [Tile : Int] = [:]
        for pos in active {
            for n in pos.allNeighbours() {
                nCount[n, default: 0] += 1
            }
        }

        var newActive: Set<Tile> = active.filter { [1,2].contains(nCount[$0]) }

        newActive.forEach {
            nCount.removeValue(forKey: $0)
        }

        nCount.filter { $0.value == 2 }.forEach {
            newActive.insert($0.key)
        }
