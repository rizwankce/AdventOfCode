...
func partOne() -> Int {
    var result = 0
    input.forEach { current in
        let com = current.components(separatedBy: " ")
        let times = com[0].components(separatedBy: "-")
        let min = Int(times[0])!
        let max = Int(times[1])!
        let letter = com[1].dropLast()
        let pwd = com[2]
        let count = pwd.filter { String($0) == letter }.count
...
func partTwo() -> Int {
    ...
    input.forEach { current in
        let com = current.components(separatedBy: " ")
        let times = com[0].components(separatedBy: "-")
        let pos1 = Int(times[0])! - 1
        let pos2 = Int(times[1])! - 1
        let letter = com[1].dropLast()
        let pwd = Array(com[2])
        let char1 = String(pwd[pos1])
        let char2 = String(pwd[pos2])
