...
func partOne() -> Int {
    func count(_ group: [String]) -> Int {
        let set = Set(group.flatMap { $0 })
        return set.count
    }
    var result = 0
    input.forEach { group in
        result += count(group)
    }
    return result
}
...
func partTwo() -> Int {
    func count(_ group: [String]) -> Int {
        if group.count == 1 {
            return Set(group.flatMap { $0 }).count
        }
        var c: Set<Character> = Set(group.first!)
        group.forEach {
            let set = Set($0)
            c = c.intersection(set)
        }
