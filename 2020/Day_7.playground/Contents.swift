...
input.forEach { line in
    let com = line.components(separatedBy: "bags contain")
    let key = com[0].replacingOccurrences(of: " ", with: "")
    let value = com[1].components(separatedBy: ",").map {
...
func partOne() -> Int {
    var current: Set<String> = ["shinygold"]
    ...
    while !current.isEmpty {
        ...
        for (k,v) in bags {
            current.forEach { c in
                if !valid.contains(k) {
                    if v.filter({ $0.contains(c) }).count != 0 {
                        next.insert(k)
                    }
                }
            }
...
func partTwo() -> Int {
    func get(_ type: String) -> Int {
        var count = 0
        if let list = bags[type],
            list.count > 0 {
            list.forEach {
                if $0 != "noother" {
                    let num = Int(String($0.first!))!
                    count +=  num + get(String($0.dropFirst())) * num
                }
            }
        }
        return count
