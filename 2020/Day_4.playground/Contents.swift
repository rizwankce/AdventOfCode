...
func partOne() -> Int {
    ...
    input.forEach { (passport) in
        let test = fields.reduce(true) { (r, field) -> Bool in
            return r && passport.contains(field)
        }
        if test {
            result += 1
        }
...
func partTwo() -> Int {
    ...
    func check(_ passport: String) -> Bool {
        var dict: [String: String] = [:]
        passport.components(separatedBy: " ").map {
            let v = $0.components(separatedBy: ":")
            dict[v[0]] = v[1]
        }

        var test = true
        if let byr = dict["byr"] {
            test = test && (1920...2002).contains(Int(byr)!)
        }
        if let iyr = dict["iyr"] {
            test = test && (2010...2020).contains(Int(iyr)!)
        }
        if let iyr = dict["eyr"] {
            test = test && (2020...2030).contains(Int(iyr)!)
        }
        if let hgt = dict["hgt"] {
            if hgt.contains("cm") {
                let h = hgt.replacingOccurrences(of: "cm", with: "")
                test = test && (150...193).contains(Int(h)!)
            }
            else if hgt.contains("in") {
                let h = hgt.replacingOccurrences(of: "in", with: "")
                test = test && (59...76).contains(Int(h)!)
            }
            else {
                test = false
            }
        ...
        if let hcl = dict["hcl"] {
            test = test && hcl.range(of: "^#[0-9a-f]{6}$", options: .regularExpression) != nil
        }
        if let ecl = dict["ecl"] {
            test = test && ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(ecl)
        }
        if let pid = dict["pid"] {
            test = test && pid.range(of: "^[0-9]{9}$", options: .regularExpression) != nil
        }
        return test
    }
