...
var allFoods: [(Set<String>, Set<String>)] = []
for line in input {
    let com = line.components(separatedBy: " (contains ")
    let ing = com[0].components(separatedBy: " ")
    let alg = com[1].replacingOccurrences(of: ")", with: "")
        .replacingOccurrences(of: ",", with: "")
        .components(separatedBy: " ")
    allFoods.append((Set(ing),Set(alg)))
}

var allIng: Set<String> = []
var allAlg: Set<String> = []

for (i,a) in allFoods {
    i.forEach { allIng.insert($0) }
    a.forEach { allAlg.insert($0) }
}

var couldBe: [String : Set<String>] = [:]
var count: [String : Int] = [:]
for i in allIng {
    couldBe[i] =  Set(allAlg)
}

for (i,a) in allFoods {
    for ing in i {
        count[ing, default: 0] += 1
    }

    for alg in a {
        for ing in allIng {
            if !i.contains(ing) {
                if var s = couldBe[ing] {
                    s.remove(alg)
                    couldBe[ing] = s
                }
            }
...
func partTwo() -> String {
    ...
    return coms.sorted { $0.value < $1.value}
    .map(\.key)
    .joined(separator: ",")
}
